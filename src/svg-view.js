...
function SvgView({dimensions, location, direction}) {
  ...
  const renderCell = row => cell => {
    ...
    return (
      <svg key={key} width="100" height="100">
        <rect x="5" y="5" width="90" height="90" rx="15" ry="15" fill="none" stroke="royalblue" strokeWidth="2" />
        {active && <circle cx="50" cy="50" r="40" fill="#ffff33" />}
        {active && <line x1="50" y1="50" x2={x2} y2={y2} stroke="#282c34" strokeWidth="3" />}
      </svg>
    )
  }
...
const getDirectionPoint = direction => {
  switch (direction) {
    case 'north': return [50, 10]
    case 'east': return [90, 50]
    case 'south': return [50, 90]
    case 'west': return [10, 50]
